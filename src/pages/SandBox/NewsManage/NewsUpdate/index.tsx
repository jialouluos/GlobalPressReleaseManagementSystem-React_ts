...
export default function NewsUpdate(props: IProps) {
    const [CurrentIndex, setCurrentIndex] = useState(0)
    const [Categories, setCategories] = useState<ICategories[]>([])
    const navigate = useNavigate();
    const match = useMatch('/sandbox/news-manage/update/:id')
    ...
    const handlerUpdate = (state: number) => {
        axios.patch(`news/${NewsFromData.id}`, {
            "content": NewsFromData.content,
            "title": NewsFromData.title,
            "categoryId": NewsFromData.categoryId,
            "auditState": state
        }).then(res => {
    ...
    useEffect(() => {
        ...
        NewFromRef.current?.setFieldsValue(NewsFromData)
    }, [NewsFromData])
    ...
    return (
        <div>
            ...
            <div style={{ marginTop: "50px" }} className={CurrentIndex !== 2 ? style.hidden : ""}>
                {CurrentIndex === 2 && <Button type="primary" onClick={() => { handlerUpdate(0); }}>保存到草稿箱</Button>}
                {CurrentIndex === 2 && <Button style={{ float: "right" }} danger onClick={() => { handlerUpdate(1); }}>提交审核</Button>}
            </div>
